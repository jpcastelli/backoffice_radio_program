...
;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype = {
		init: function () {
				...
				this._theform = $(this.inputelement).parents("#crop-form");
				if(this._theform.length) {
					
				}
                                $("#submit-crop").click(function(){ return _this._formsubmit(); });
						...
						_this.options.formSubmitted();
                                        ...
                                        request.onreadystatechange = function(e) {
                                            if (request.status == 200) {
                                                ...
                                                var d = new Date();
                                                var time = d.getTime();
                                                var coverFolder = '/uploads/posts/cover/';
                                                $('#post_cover').val(src);
                                                $('#cover-image-choosen').html('<a href="#" id="removeCover">Eliminar</a><img src="'+ coverFolder + src +'?'+time+'" />');
                                                $('#choose-cover').toggle();
